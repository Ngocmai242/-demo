...
def get_recommended_outfits(gender, occasion, style, body_shape, budget, garment_type="any", limit=6, require_clean=False):
    """
    ...
    try:
        ...
        if require_clean:
            query = query.outerjoin(NormalizedProduct, Product.id == NormalizedProduct.product_id)
            query = query.filter(or_(Product.clean_image_path.isnot(None), NormalizedProduct.status == 'processed'))
        ...
        def _garment_synonyms(gt: str) -> list[str]:
            ...
            if gt in ("dress", "dresses", "đầm", "váy"):
                return ["dress", "one-piece", "vay", "dam", "váy liền"]
            if gt in ("tops", "top", "ao", "áo"):
                return ["tops", "top", "ao", "shirt", "tee", "tshirt", "blouse", "hoodie", "sweater"]
            if gt in ("bottoms", "bottom", "quần", "quan"):
                return ["bottoms", "bottom", "quan", "skirt", "jeans", "pants", "short", "chân váy"]
            return [gt]
        ...
        for p in products:
            ...
            results.append({
                ...
                'name': p.name,
                'price': p.price,
                'image_url': best_clean if best_clean else p.image_url,
                ...
                'photo_type': norm_entry.photo_type if norm_entry and norm_entry.photo_type else 'model'
            })
        return results
    except Exception as e:
...
def _resolve_clean_abs(clean_rel, static_folder_path):
    import os
    if not clean_rel: return None
    try:
        rel = str(clean_rel).lstrip("/").replace("\\", "/")
...
def _run_vton_pipeline_v2(in_path, garments, results_dir, out_path, static_folder_path, gender="female"):
    ...
    is_fb = (len(tried_items) == 0)
    print(f"[TRYON] [{time.time() - t_start:.1f}s] ✅ Hoàn tất Try-On (FB={is_fb}) -> {out_path}")
    return {"path": out_path, "is_fallback": is_fb, "tried_items": tried_items, "error_message": last_error_msg}

@main_bp.route('/api/recommend-products', methods=['POST'])
def recommend_products_api():
    try:
        req_json = request.get_json(silent=True) or {}
        outfit_type = (request.form.get('outfit_type') or req_json.get('outfit_type') or 'dress').strip()
        gender = request.form.get('gender') or req_json.get('gender') or 'female'
        occasion = request.form.get('occasion') or req_json.get('occasion') or 'casual'
        style = request.form.get('style') or req_json.get('style') or 'any'
        
        # Đồng bộ hóa garment_type với outfit_type từ frontend
        garment_type = outfit_type 
        ...
        if outfit_type == 'dress':
            # Chỉ lấy đầm/váy liền (không lấy chân váy/áo)
            dresses = [p for p in valid_products if any(k in (str(p.get("garment_type") or "") + " " + str(p.get("name") or "")).lower() for k in ["dress", "one-piece", "váy liền", "đầm", "vay lien", "dam"])]
            results = [{"type": "single", "items": [d]} for d in dresses]
            
        elif outfit_type == 'top':
            # Loại bỏ váy/quần khỏi danh sách áo
            tops = [p for p in valid_products if any(k in (str(p.get("garment_type") or "") + " " + str(p.get("name") or "")).lower() for k in ["top", "ao", "áo", "shirt", "t-shirt", "vest", "khoác"])]
            tops = [p for p in tops if not any(k in (str(p.get("garment_type") or "") + " " + str(p.get("name") or "")).lower() for k in ["váy", "đầm", "dress", "pants", "quần", "skirt"])]
    ...
    if task_id in vton_processor_queue.results:
        ...
        if result["status"] == "success":
            ...
            return jsonify({
                ...
                "tried_items": final_data.get("tried_items", [])
            }), 200
        else:
            return jsonify({
                "success": True,
